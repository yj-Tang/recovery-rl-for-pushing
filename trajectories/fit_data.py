...
def get_training_data(data):
    ...
    for i in range(0, time.shape[0]-1):
        ...
        vel_r_x_oframe = calc_object_frame_x(vel_r_x, vel_r_y, -theta_o)
        vel_r_y_oframe = calc_object_frame_y(vel_r_x, vel_r_y, -theta_o)
        ...
        vel_o_x_oframe = calc_object_frame_x(vel_o_x, vel_o_y, -theta_o)
        vel_o_y_oframe = calc_object_frame_y(vel_o_x, vel_o_y, -theta_o)
        ...
        pos_r_x_oframe_diff = calc_object_frame_x(pos_r_x_diff, pos_r_y_diff, -theta_o)
        pos_r_y_oframe_diff = calc_object_frame_y(pos_r_x_diff, pos_r_y_diff, -theta_o)
        ...
        action_x_oframe = calc_object_frame_x(action_x, action_y, -theta_o)
        action_y_oframe = calc_object_frame_y(action_x, action_y, -theta_o)
        action_theta_oframe = action_theta

        # input data in the object frame
        ...
        d_px_oframe = calc_object_frame_x(d_px, d_py, -theta_o)
        d_py_oframe = calc_object_frame_y(d_px, d_py, -theta_o)
        d_theta_oframe = d_theta
        d_vx_oframe = calc_object_frame_x(d_vx, d_vy, -theta_o)
        d_vy_oframe = calc_object_frame_y(d_vx, d_vy, -theta_o)
        d_omega_oframe = d_omega

        # output data in the object frame
        learning_y = np.array( [ d_px_oframe, d_py_oframe, d_theta_oframe,
            d_vx_oframe, d_vy_oframe, d_omega_oframe
            ] )
...
def main(total_data):
    ...
    torch.save(model.state_dict(), "saved_nn/torch_model_supervised.pt")
    print("Done training!")
...
def plot_trajectory(X_true, object_states, i):
    ...
    plt.ylabel("y [m]")
    plt.legend()
    plt.savefig("plots_results/traj_trained_nn" + str(i) + ".png")
...
def plot_predictions(y_train, y_pred, i):
    ...
    plt.ylabel("y [m]")
    plt.legend()
    plt.savefig("plots_results/pred_x_y" + str(i) + ".png")


def to_world(y_pred, object_state):
    # y_pred = [dx, dy, dtheta, dvx, dvy, domega]
    theta_o = object_state[2]

    d_px = calc_object_frame_x(y_pred[0], y_pred[1], theta_o)
    d_py = calc_object_frame_y(y_pred[0], y_pred[1], theta_o)
    d_theta = y_pred[2]
    d_vx = calc_object_frame_x(y_pred[3], y_pred[4], theta_o)
    d_vy = calc_object_frame_y(y_pred[3], y_pred[4], theta_o)
    d_omega = y_pred[5]

    y_pred_world = np.array([d_px, d_py, d_theta, d_vx, d_vy, d_omega])
...
def update_X_input(new_object_state, X_true, n):
    ...
    pos_r_x_diff = state_robot[0] - state_object[0]
    pos_r_y_diff = state_robot[1] - state_object[1]
    pos_r_theta_diff = state_robot[2] - state_object[2]
    ...
    X_test_new  = np.array( [vel_r_x_oframe, vel_r_y_oframe, vel_r_theta_oframe, pos_r_x_oframe_diff, pos_r_y_oframe_diff, pos_r_theta_oframe_diff,
         vel_o_x_oframe, vel_o_y_oframe, vel_o_theta_oframe, 
         action_x_oframe, action_y_oframe, action_theta_oframe]
          )
...
def test(total_data):
    ...
    for i in lst_test:
        ...
        model.load_state_dict(torch.load("saved_nn/torch_model_supervised.pt"))
        ...
        print("object_state START, ", object_state)
        ...
        y_test_total_world = np.zeros((y_test.shape[0]-1, y_test.shape[1]))
        ...
        for n in range(0, y_test.shape[0]-1):
            ...
            # Convert the prediction to world frame 
            y_pred_world = to_world(y_pred, object_state)
            y_test_world = to_world(y_test[n, :], object_state)

            # Save data
            y_pred_total[n, :] = y_pred
            y_pred_total_world[n, :] = y_pred_world
            y_test_total_world[n, :] = y_test_world
            print("pred: ", y_pred_total[n,:], "true: ", y_test[n, :])
            print("pred (world): ", y_pred_total_world[n,:], "true (world): ", y_test_total_world[n, :])

            # Calc new object state 
            new_object_state = object_state + y_pred_world
        ...
        plot_predictions(y_test_total_world, y_pred_total_world, temp[0])

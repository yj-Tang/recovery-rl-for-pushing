...
def cylinder_pushing_dynamics_mpc(x, u, params):
    pr = Problem()
    index = Index()
    ...
    # for the robot
    d_rob_state = cd.vertcat(cos_theta_r*v_r, 
                            sin_theta_r*v_r, \
                            d_theta_r, \
                            d_v_r, \
                            d_omega_r
    ...
    d_C_q = cd.vertcat(-cos_theta_o, -sin_theta_o)
    diata_pos = d_C_q * (1.0 / cd.dot(d_C_q, d_C_q) * (-Con))
    ...
    d_dC_dq = d_C_q
    diata_d_pos = d_dC_dq * ( 1.0 / cd.dot(d_dC_dq, d_dC_dq) * (-d_Con))
    ...
    for i in range(4):  # for the object with four legs
        ...
        sig_v_O_x =cd.vertcat(utils.sigmoid_adap(-d_theta_o*(O_y*cos_theta_o + O_x*sin_theta_o - a)), \
    ...
    F_fg = -mu_g * obj_mass * grav_acc / 4 * cd.vertcat(inte_f[0], inte_f[1])
    ...
    # unknowns = cd.dot(np.linalg.inv(S), DAE_right)
    unknowns = cd.solve(S, DAE_right)
    dd_x = unknowns[0]
    dd_y = unknowns[1]
    dd_theta = unknowns[2]
    O_F_xp = unknowns[3]
    ...
    d_y = vy_o
    d_theta = d_theta_o
    d_obj_state = cd.vertcat(d_x, d_y, d_theta, dd_x, dd_y, dd_theta)

    d_state = cd.vertcat(d_rob_state, d_obj_state)
